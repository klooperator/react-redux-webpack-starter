...
export function getInitData(successAction, failureAction, dispatch){
    return Promise.all([getCurrentUser(),getAllBoxes(),getCurrentBox(),walletUserTransactions()])
    .then(values => {
        /*console.log('all promises resolved');
        console.log(values);*/
        return Promise.all([values[0].json(), values[1].json(), values[2].json(), values[3].json()]);
    }).then(values=>{
        /*console.log('all promises resolved again');
        console.log(values);*/
        let finalJson = {currentUser:values[0], userBoxes:values[1], currentBox:values[2],wallet:values[3],ok:true};
